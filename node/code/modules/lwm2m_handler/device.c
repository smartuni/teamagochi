 ...
 */
static lwm2m_obj_device_t _device_object;

/**
 * @brief Pool of object instances.
 */
static lwm2m_obj_pet_inst_t _instances[CONFIG_LWM2M_PET_DEVICE_INSTANCES_MAX];

lwm2m_object_t *lwm2m_object_device_init(lwm2m_client_data_t *client_data)
{
    assert(client_data);
    int res = lwm2m_object_device_init_derived(client_data,
                                            &_device_object,
                                            _device_object,
                                            _instances,
